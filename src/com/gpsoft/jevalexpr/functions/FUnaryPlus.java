...
public class FUnaryPlus extends Function {
	public FUnaryPlus(String name) {
		super();
		...
		this.typeStep =TypeStep.E_unary_plus; 
		this.stepRef = 0;
	...
	public boolean check(ExpBin<?> expBin, int idxStep) {
		...
		if (step.getOpnd().size() != 1 ) {
			Logger.error("Function unary_plus (+) work with one argument not with " + step.getOpnd().size() + ".");
			return false;
		}
		...
		if ( expBin.getStep().get(idxOpd1).getResType() != TypeData.E_int &&
			...
			Logger.error("function unary_plus (+) work only with number argument.");
			return false;
		}
	...
	public boolean exec(ExpBin<?> expBin, int idxStep) {
		Logger.debug("In Exec FUnaryPlus");	
		Step<?> step = expBin.getStep().get(idxStep);
		...
		if ( Utility.isDouble(expBin.getStep().get(idxOpd1).getData().getValue()) ) {
		    ...
		    expBin.getStep().get(idxStep).setData(new DataValue<Double>(valueOp1));
     		expBin.getStep().get(idxStep).setResType(TypeData.E_double);
		...
		if ( Utility.isInteger(expBin.getStep().get(idxOpd1).getData().getValue()) ) {
		    ...
		    expBin.getStep().get(idxStep).setData(new DataValue<Integer>(valueOp1));
     		expBin.getStep().get(idxStep).setResType(TypeData.E_int);
		    expBin.getStep().get(idxStep).setNull(false);
		}
