...
public class FLpad extends Function{
	...
	public boolean check(ExpBin<?> expBin, int idxStep) {
		...
		if (step.getOpnd().size() < 2 || step.getOpnd().size() > 3 ) {
			Logger.error("Function lpad work with two or three arguments not with " + step.getOpnd().size() + ".");
			return false;
		}
		...
		if ( expBin.getStep().get(idxOpd).getResType() != TypeData.E_string ) {
			Logger.error("function lpad first argument must be a string.");
			return false;
		}
		...
		if ( expBin.getStep().get(idxOpd2).getResType() != TypeData.E_int ) {
			Logger.error("function lpad second argument must be an integer.");
			return false;
		}
		...
		if ( step.getOpnd().size() == 3 ) {
			...
			if ( expBin.getStep().get(idxOpd3).getResType() != TypeData.E_string ) {
				Logger.error("function lpad third argument must be a string.");
				return false;
			}			
		}
	...
	public boolean exec(ExpBin<?> expBin, int idxStep) {
		...
		if ( expBin.getStep().get(idxOpd1).getResType() == TypeData.E_string &&
			...
			if ( length < 0 ) {
				Logger.error("function lpad second argument can't be negative.");
				return false;
			}
			...
			String ris = Utility.lpad(value, length, filler);
