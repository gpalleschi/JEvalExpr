...
public class FRound extends Function{
	...
	public FRound() {
		...
		this.typeStep =TypeStep.E_round; 
		this.stepRef = 0;
	...
	public boolean check(ExpBin<?> expBin, int idxStep) {
		...
		if (step.getOpnd().size() != 1 && step.getOpnd().size() != 2) {
			Logger.error("Function round work with one or two arguments not with " + step.getOpnd().size() + ".");
			return false;
		}
		...
		if ( expBin.getStep().get(idxOpd).getResType() != TypeData.E_double &&
			...
			Logger.error("function round first argument must be numeric.");
			return false;
		}
		...
		if ( step.getOpnd().size() == 2 ) {
			...
			if ( expBin.getStep().get(idxOpd2).getResType() != TypeData.E_int ) {
				Logger.error("function round second argument must be integer.");
				return false;
			}
		}
