...
public class FCeil extends Function{
	...
	public boolean check(ExpBin<?> expBin, int idxStep) {
		...
		if (step.getOpnd().size() != 1 && step.getOpnd().size() != 2) {
			Logger.error("Function ceil work with one or two arguments not with " + step.getOpnd().size() + ".");
			return false;
		}
		...
		if ( expBin.getStep().get(idxOpd).getResType() != TypeData.E_double &&
			...
			Logger.error("function ceil first argument must be numeric.");
			return false;
		}
		...
		if ( step.getOpnd().size() == 2 ) {
			...
			if ( expBin.getStep().get(idxOpd2).getResType() != TypeData.E_int ) {
				Logger.error("function ceil second argument must be integer.");
				return false;
			}
		}
