...
public class FSum extends Function{
	...
	private double execSum(Double op1, Double op2) {
		return op1 + op2;
	}
	
	private double execSum(Double op1, Integer op2) {
		return op1 + op2;
	}
	
	private double execSum(Integer op1, Double op2) {
		return op1 + op2;
	}
	
	private int execSum(Integer op1, Integer op2) {
		return op1 + op2;
	}
	...
	public boolean exec(ExpBin<?> expBin, int idxStep) {
		...
		if ( Utility.isDouble(expBin.getStep().get(idxOpd1).getData().getValue()) &&
		    ...
		    expBin.getStep().get(idxStep).setData(new DataValue<Double>(execSum(valueOp1, valueOp2)));
     		expBin.getStep().get(idxStep).setResType(TypeData.E_double);
		...
		if ( Utility.isInteger(expBin.getStep().get(idxOpd1).getData().getValue()) &&
		    ...
		    expBin.getStep().get(idxStep).setData(new DataValue<Integer>(execSum(valueOp1, valueOp2)));
     		expBin.getStep().get(idxStep).setResType(TypeData.E_int);
		    expBin.getStep().get(idxStep).setNull(false);
		}
	/*	
