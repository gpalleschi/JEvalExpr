...
public class FAtan extends Function{
	...
	public boolean check(ExpBin<?> expBin, int idxStep) {
		...
		if (step.getOpnd().size() != 1 ) {
			Logger.error("Function atan work with one argument not with " + step.getOpnd().size() + ".");
			return false;
		}
		...
		if ( expBin.getStep().get(idxOpd).getResType() != TypeData.E_double &&
			...
			Logger.error("function atan work only with a numeric argument.");
			return false;
		}
	...
	public boolean exec(ExpBin<?> expBin, int idxStep) {
    	...
    	expBin.getStep().get(idxStep).setData(new DataValue<Double>(Math.atan(value)));
