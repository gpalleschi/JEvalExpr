...
public class FMod extends Function{

	public FMod(String name) {
		super();
	...
	private double execMod(Double op1, Double op2) {
		return op1 % op2;
	}
	
	private double execMod(Double op1, Integer op2) {
		return op1 % op2;
	}
	
	private double execMod(Integer op1, Double op2) {
		return op1 % op2;
	}
	
	private int execMod(Integer op1, Integer op2) {
		return op1 % op2;
	}
	
	public boolean exec(ExpBin<?> expBin, int idxStep) {
		Logger.debug("In Exec FMod");	
		Step<?> step = expBin.getStep().get(idxStep);
		...
		Logger.debug("In FMod index Opd1 : " + idxOpd1);
		Logger.debug("In FMod index Opd2 : " + idxOpd2);
		...
		if ( Utility.isDouble(expBin.getStep().get(idxOpd1).getData().getValue()) &&
		    ...
		    expBin.getStep().get(idxStep).setData(new DataValue<Double>(execMod(valueOp1, valueOp2)));
     		expBin.getStep().get(idxStep).setResType(TypeData.E_double);
		...
		if ( Utility.isInteger(expBin.getStep().get(idxOpd1).getData().getValue()) &&
		    ...
		    expBin.getStep().get(idxStep).setData(new DataValue<Integer>(execMod(valueOp1, valueOp2)));
     		expBin.getStep().get(idxStep).setResType(TypeData.E_int);
		    expBin.getStep().get(idxStep).setNull(false);
		}
